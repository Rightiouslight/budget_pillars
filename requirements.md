...
## 2. Core Firestore Data Locations

The application's data is stored in Firestore. Below are the primary collection and document paths.

- **Monthly Budgets**: `users/{user_uid}/budgets/{yyyy-MM}`

  - **Description**: The root document for a specific month's budget. Contains all accounts, cards, transactions, and settings for that period.

- **User Settings**: `users/{user_uid}/data/settings`

  - **Description**: A single document storing the user's personal preferences like currency, theme, and budget start day.

- **Share Invitations**: `shares/{share_id}`

  - **Description**: A top-level collection holding all invitation documents. This is queried to find pending invitations for a user.

- **Shared Access Grant**: `users/{user_uid}/sharedAccess/{owner_uid}`
  - **Description**: A document that grants the current user (`user_uid`) access to another user's budget (`owner_uid`). This populates the "Switch Budget" menu.
- **Shared With Information**: `users/{owner_id}/sharedWith/{recipientId}`

  - **Description**: A document created under the budget owner's data to record who they have shared their budget with.

- **User Profile Data**: `users/{user_uid}/data/userData`
  - **Description**: A single document storing non-sensitive user profile information like display name and email, nested under the user's main document.
...
### 4.1. Firestore Data Models (with `freezed` and `json_serializable`)
...
### 4.2. Firebase Repository Pattern
...
## 5. UI/UX and Feature Implementation

### 5.1. Main Dashboard (Kanban View)
...
- [ ] **Feature Goal**: Create a dedicated screen or dialog, accessible per-account, that provides a high-level overview for rapid budget adjustments.
- [ ] **UI/Layout**:
  - Display a summary card at the top showing: `Total Income`, `Total Budgeted`, and `Remaining (Income - Budgeted)`. These values must update in real-time as the user types.
  - Show a scrollable list of all `Category` cards belonging to the selected account.
  - Each list item should display the category's icon, name, and an input field for its `budgetValue`.
- [ ] **State Management**:
  - The `BudgetPlannerController` should be a `Notifier` that holds the temporary state of category budget values.
  - `Total Income` is calculated by summing all `Pocket` balances and any unprocessed `RecurringIncome` for that account.
  - `Total Budgeted` is calculated by summing the `budgetValue` of all categories in the planner's state.
- [ ] **Logic**:
